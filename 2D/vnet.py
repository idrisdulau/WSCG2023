...
class Vnet(torch.nn.Module):
    ...
    def singleBlock(inChan, outChan):
        ...
        torch.nn.Conv2d(inChan, outChan, kernel_size=5, stride=1, padding=2),
    ...
    def doubleBlock(inChan, outChan):
        ...
        torch.nn.BatchNorm2d(outChan),
        torch.nn.PReLU(),
        torch.nn.Conv2d(outChan, outChan, kernel_size=5, stride=1, padding=2),
        torch.nn.BatchNorm2d(outChan),
        torch.nn.PReLU())

    def outputBlock(inChan, outChan):
        return torch.nn.Sequential(
        torch.nn.Conv2d(inChan, outChan, kernel_size=1))

    def convBlock(inChan, outChan):
        return torch.nn.Sequential(
        torch.nn.Conv2d(inChan, outChan, kernel_size=2, stride=2))  
